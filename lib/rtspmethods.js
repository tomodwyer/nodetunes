...
module.exports = function rtspMethods(rtspServ) {
  ...
  const setup = function setup(req, res) {
    ...
    if (rtspServer.ports.length >= 3) {
      ...
      res.set(
        "Transport",
        `RTP/AVP/UDP;unicast;mode=record;server_port=${rtspServer
          .ports[0]};control_port=${rtspServer
          .ports[1]};timing_port=${rtspServer.ports[2]}`
      );
      res.set("Session", "1");
